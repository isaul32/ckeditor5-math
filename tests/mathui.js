...
describe('MathUI', () => {
	...
	describe('init', () => {
		it('should register click observer', () => {
			expect(
				editor.editing.view.getObserver(ClickObserver),
			).to.be.instanceOf(ClickObserver);
		});
	...
	describe('_showUI()', () => {
		...
		it('should add #mainFormView to the balloon and attach the balloon to the selection when text fragment is selected', () => {
			setModelData(editor.model, '<paragraph>f[o]o</paragraph>');
			const selectedRange = editorElement.ownerDocument
				.getSelection()
				.getRangeAt(0);
		...
		describe('keyboard support', () => {
			...
			it('should make stack with math visible on Ctrl+M keystroke - math', () => {
				setModelData(
					editor.model,
					'<paragraph><$text equation="x^2">f[]oo</$text></paragraph>',
				);
		...
		describe('mouse support', () => {
			...
			it('should not hide the UI upon clicking inside the the UI', () => {
				...
				mathUIFeature._showUI();
				balloon.view.element.dispatchEvent(
					new Event('mousedown', { bubbles: true }),
				);
		...
		describe('math form view', () => {
			...
			describe('binding', () => {
				...
				it('should execute math command on mainFormView#submit event', () => {
					...
					expect(
						formView.mathInputView.fieldView.element.value,
					).to.equal('x^2');
